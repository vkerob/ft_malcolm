...
{
	int	 i = 1;
	char spoofed_mac[MAC_ADDR_LEN] = { 0 };
	...
	while (i < *argc && (*argv)[i][0] == '-')
	{
		...
		else if (ft_strncmp((*argv)[i], "--attack", 8) == 0)
		{
			...
			if ((*argv)[i][8] == '=' && (*argv)[i][9] != '\0')
			{
				...
				ft_strlcpy(spoofed_mac, &(*argv)[i][9], MAC_ADDR_LEN);
				printf(COLOR_CYAN
					   "[Info] Spoofed MAC provided: %s\n" COLOR_RESET,
					   spoofed_mac);
			}
			else
			{
	...
	if (spoofed_mac[0] != '\0')
	{
		ft_strlcpy(args->spoofed_mac, spoofed_mac, MAC_ADDR_LEN);
	}
